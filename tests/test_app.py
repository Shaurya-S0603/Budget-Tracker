...
def test_navigation_and_weekly_save_survive_new_session(database):
    ...
    assert any(("Cloud database" if database == "postgresql" else "Local storage")
               in caption.value for caption in fresh.sidebar.caption)


def test_bad_connection_displays_safe_error_and_stops(tmp_path, monkeypatch):
    monkeypatch.setenv("DATABASE_URL", "not-a-database-url-with-secret")
